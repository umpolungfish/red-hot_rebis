...
- **SMILES:** `ClCCN(CCCl)C(=O)OCc1ccccc1SSc1ccccc1COC(=O)N(CCCl)CCCl` (representative structure built to
  the source architecture; the source specifies the design as an abstract two-domain mechanophore, not a
  literal SMILES). The two aryl groups are the binding domains, the central disulfide is the
  strain-cleavable tether (the 𐑹 element that breaks under malignant asymmetry), and the two
  self-immolative benzyl carbamates mask nitrogen-mustard payloads that release once the tether cleaves.
