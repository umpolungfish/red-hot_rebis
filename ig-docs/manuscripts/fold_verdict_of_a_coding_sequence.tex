...
\label{def:pairing}
Read the word as a cycle. Scan forward around it; on each $\in$ push its position
onto a stack, and on each $\ni$ pop, forming a pair whose \emph{region} runs forward
around the cycle from the fork to the fuse. Positions left unpopped at the end of a
full traversal are the \emph{unmatched forks}; fuses that found an empty stack after
a full traversal are the \emph{unmatched fuses}.
\end{definition}
...
\begin{lemma}[Surplus]
\label{lem:surplus}
For any word $w$, at most one of the two unmatched sets is non-empty, and
\[
  \#\{\text{unmatched forks}\} - \#\{\text{unmatched fuses}\}
  \;=\; |w|_{\in} - |w|_{\ni}.
\]
\end{lemma}

\begin{proof}
Each pair consumes one fork and one fuse, so the displayed identity is immediate
from counting. For the first claim, suppose both sets were non-empty, and let $i$ be
the position of an unmatched fork and $j$ that of an unmatched fuse. Traverse the
cycle forward from $i$. The fork at $i$ is pushed and, by hypothesis, never popped,
so the stack is non-empty at every position strictly after $i$ until the traversal
returns to $i$. The position $j$ is reached during that interval, since the cycle
visits every position. At $j$ the stack is non-empty, so the fuse pops, and $j$ is
matched, contrary to hypothesis.
\end{proof}
...
and $21$ at $F$. \Cref{thm:closed} reproduces all sixty-five values.
