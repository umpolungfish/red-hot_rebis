...
info_line(f"  MHET: cuts={len(r.get('cuts',[]))}")
...
info_line(f"  [{'✓' if b4_complement(t)==Belnap.B else '✗'}] B4: T complement B")
info_line(f"  [{'✓' if b4_complement(f)==Belnap.N else '✗'}] B4: F complement N")
info_line(f"  [{'✓' if band(t,t)==t and band(f,t)==f and bor(t,f)==t else '✗'}] Belnap")
info_line(f"  [✓] Codons: {len(CODON_CATALOG)}")
info_line(f"  [✓] ParaVM ready")
